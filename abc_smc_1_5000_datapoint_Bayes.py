...
import model_equation as model_equation
...
def GeneratePar(x_data, iter,
  ...
  # until a suitable parameter(below the threshold) is found
    ...
    if (evaluateprior(proposed_pars, model_equation.model) > 0):  #if d < eps_dist
        # here
...
def main(argv):
    ...
    Sequential_ABC(x_data, ncpus=ncpuss, initial_dist=initdist, final_dist=finaldist, prior_label=pl, Npars=10000, #number of sampling time#Adjusted to 5000 to increase exploration
